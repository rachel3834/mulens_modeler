...
class MicrolensingEvent():
	...
	# Projected minimum separation of lens and source [units R_E, lens plane]
        self.u_o = None
	...
	# Projected separation of lens and source [units R_E, lens plane] as a function of time
	self.u_t = None
	
	# Source-lens relative velocity (assumed to be straight line motion) [ms^-1]
	self.v_SL = None
	
	# Arrays of the lens-source relative motion in the lens plane as functions
	# of time:
	self.x_lens = None
	self.y_lens = None
	...
	# big_omega = parameter of orbital motion of Earth around the Sun projected into the lens plane
	self.t_c = None
	self.t_p = None
	self.omega = None
	self.big_omega_o = ( 2.0 * np.pi ) / 365.25       # [rads day^-1] for consistency with other parameters
	self.omega_dt = None
	...
	# Radius of Swift's orbit from Earth centre (converting nautical mile altitude->m):
	self.swift_orbit_radius = ( 324.0 * 1852.0 ) + R_earth
	self.swift_orbit_period = np.sqrt( ( 4.0 * np.pi**2 * self.swift_orbit_radius**3 ) / ( G * M_earth ) )
	...
	A_t = ( self.u_t * self.u_t + 2 ) / ( self.u_t * np.sqrt( self.u_t * self.u_t + 4 ) )
        
	if model == 'pspl': self.A_t_pspl = A_t
	elif model == 'pspl_parallax': self.A_t_pspl_parallax = A_t
	elif model == 'pspl_parallax_satellite': self.A_t_pspl_parallax_satellite = A_t
	
    ###############################
    # CALCULATE EINSTEIN RADIUS
    def calc_einstein_radius(self):
        '''Method to calculate the Einstein radius of the lens in the lens plane in physical units.
	Returns R_E in [m]'''
	
	self.R_E = np.sqrt( ( 4.0 * constants.G * self.M_L * self.D_L * self.D_LS ) / ( constants.c * constants.c * self.D_S ) )


    ################################
    # CALCULATE LENS-SOURCE SEPARATION
    def calc_D_lens_source(self):
        '''Method to calculate the physical separation distance of lens and source.'''
    ...
    def calc_beta(self):
        '''Method to calculate the ecliptic latitude of the source from its coordinates.'''
	
	sky_coords = coordinates.ICRS(ra=self.RA,dec=self.Dec, unit=(units.hourangle, units.degree))
    ...
    def calc_earth_orbit(self):
        '''Method to calculate the parameterization of Earths orbital motion'''
	...
	#print 'Time of maximum light as an angular frequency (omega) = ',self.omega,' t_E = ',self.t_E
    ...
    def calc_proj_observer_pos(self,parallax=False,satellite=False):
        '''Method to calculate the position of the observer projected to the lens plane, 
	factoring in both annual parallax and satellite parallax.'''
	
	# By default, the observer is assumed to be at the centre of the Earth, 1AU away from the 
	# Sun-source line:
	if parallax == False:
	...
	if parallax == True:
	    ...
	    # Technically this currently assumes that Earth-bound observers are at the
	    # centre of the planet
	    self.calc_earth_orbit()
	
	    # Radius of the observer from the centre of the Sun, factoring in the orbit of the satellite
	    # if requested:
	    r_obs = self.earth_helio_radius
	
	# Including the additional altitude of a satellite orbiting the Earth:
	if satellite == True: r_obs = r_obs.value + self.swift_orbit_radius + ( self.omega_swift * ( self.t - self.swift_t ) )
	
	# Identify the time of closest approach of the observer to the 
	# Sun-source line:
	i = np.where( r_obs == r_obs.min() )
	self.t_c = self.t[i][0]
	#print 'Time of closest approach of observer to Sun-source line: ',self.t_c
	
	# Projecting this radius to the lens plane:
	self.alpha = ( r_obs * ( 1.0 - ( self.D_L / self.D_S ) ) ) / self.R_E
	...
	# Factoring in the ecliptic latitude, beta, to arrive at the projected observer
	# location in the lens plane as a function of time during the event:
	self.calc_beta()
	self.x_obs = self.alpha.value * np.sin(self.beta) * np.cos(self.big_omega_dtc)
	self.y_obs = self.alpha.value * np.cos(self.beta) * np.sin(self.big_omega_dtc)
      	self.r_obs = np.sqrt( ( self.x_obs*self.x_obs ) + ( self.y_obs*self.y_obs ) )
    ...
    def calc_source_lens_rel_motion(self):
        '''Method to calculate the source-lens relative motion for a generic PSPL event'''
	
	# Calculate the relative source-lens velocity [ ms^-1] based on the given t_E:
	self.v_SL = ( 2.0 * self.R_E.value ) / self.t_E.value
	#print 'Lens-source relative velocity = ',self.v_SL,'m/s'
	#print 'Einstein radius = ',self.R_E, ' crossing time = ',(self.t_E/ ( 60.0 * 60.0 * 24.0 )),'d'
	
	# Assume the motion is such that the components of it in the x, y directions
	# in the lens plane are equal.  Convert the velocity units into R_E/d:
	nu = 2.0 / ( self.t_E.value / ( 60.0 * 60.0 * 24.0 ) ) 
	self.v_SL_dx = nu * np.cos(self.phi)
	self.v_SL_dy = nu * np.sin(self.phi)
	
	# Calculate the time-dependent position of the source relative to the lens
	# in the lens plane in units of the Einstein radius:
	x_incr = ( 1.0 + 1.0 )/len(self.t)
	self.x_lens = np.arange(-1.0,1.0,x_incr)
	self.y_lens = ( self.x_lens * np.tan(self.phi) ) + ( self.u_o * np.cos(self.phi) )
	self.r_lens = np.sqrt(self.x_lens*self.x_lens + self.y_lens*self.y_lens)
      	
    ################################
    # EVENT TIMELINE
    def gen_event_timeline(self):
        '''Method to generate the event timeline, an array of timestamps for datapoints on the event lightcurve, 
	spanning the range t_o +/- (t_E/2).'''
	
	# Compute the start and end times of the event, note these are in UTC format:
    ...
    def calc_pspl_parallax_impact_param(self):
        '''Method to calculate a point-source, point-lens lightcurve given the
	lens parameters, including the parallax due to Earths orbital motion.
	Formulation derived from Buchalter, A., Kamionkowski, M., 1997, ApJ, 483, 782.
	'''
	...
	term4 = alpha_sq * ( ( np.sin( self.beta.value ) )**2 ) * ( ( np.cos(self.big_omega_dtc) )**2 ) 
	term5 = 2.0 * self.alpha.value * np.sin(self.big_omega_dtc) * ( self.omega_dt * np.sin(self.phi) + self.u_o * np.cos(self.phi) ) 
	term6 = 2.0 * self.alpha.value * np.sin(self.beta.value) * np.cos(self.big_omega_dtc) * \
		  ( self.u_o * np.sin(self.phi) - self.omega_dt * np.cos(self.phi) )

	usq_t = term1 + term2 + term3 + term4 + term5 + term6
	
	# Print these terms at t_o?  Or find when they are all minimised? # Term 3 largest at t=t_o
    ...
    def plot_lens_plane_motion(self):
        '''Method to plot the motion of the lens as a function of time in the lens plane geometry'''
	...
	# Plot the trajectory of the lens relative to the Sun-source line during the event:
	plt.plot(self.x_lens,self.y_lens,'r-')
	
	# Plot the trajectory of the observer in the lens plane during the event:
	...
	plt.plot(np.array( [self.t_o.jd-2450000.0]*2 ), np.array([-2.0,2.0]),'r-.')
